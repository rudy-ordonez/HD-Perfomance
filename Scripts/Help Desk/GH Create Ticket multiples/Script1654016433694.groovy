...
for (i = 0; i < 4500; i++) {
    ticket = WS.sendRequest(findTestObject('Postman/Help Desk/Tickets/POST CreateTicket', [('HHD_API_Url') : GlobalVariable.HHD_API_Url
                , ('HHD_API_Bearer') : GlobalVariable.HHD_API_Bearer, ('HHD_API_Tag') : GlobalVariable.HHD_API_Tag]))

    def ticket2 = ticket.getResponseText()

    //def status = WS.getResponseStatusCode(tags)
    //println(status)
